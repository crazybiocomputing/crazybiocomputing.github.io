...
function toClipboard(a1,a2,a3,a4,a5) {
  const msg = `
    <p style="font-family: Rock Salt, cursive; font-size: 14px">If you are not an expert in DNA Barcoding, look at the corresponding page in Wikipedia...</p>
    <div style="position:absolute;background:url('clipboard.svg');background-size:650px;width:650px;height:850px;">
    <form style="position:relative;margin:150px 0px 0px 60px;width:450px;font-family:'Rock Salt',cursive;font-size:20px;" action="myforensics_lab.php" method="post">
    DNA Barcode is a unique pattern of <input type="text" name="a1" size="8" value="${a1}"> 
    sequence specific of a living thing. 
    In this technique, short, highly variable regions are of interest. 
    The sequences are extracted from genomes located in <input type="text" size="10"  name="a2" value="${a2}"> 
    for animals and <input type="text" size="10" name="a3" value="${a3}"> for plants. 
    These sequences must be amplified with <input type="text" size="25"  name="a4" value="${a4}">. 
    For animals, a region of the gene named <input type="text"  name="a5" value="${a5}"> 
    is studied...<br>
    <input style="float:right" type="submit" name="OK" value="OK"></input>
    </div>
    </form>
  `;
